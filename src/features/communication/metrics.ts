...
export const locationMetricSpecs: readonly MetricSpec[] = [
  ...
  simple("drv_err", "Drive error ID1..4", "drv_err1..4", "", locationTip("四轮驱动目标与反馈误差，语义为 drvCmd - drvFb；ID1右前、ID2右后、ID3左前、ID4左后。", "接近 0，运动时随闭环快速收敛。", "持续偏大表示该轮驱动反馈不跟随命令。", "按同 ID 对比 Drive command、Drive feedback、DGM age 和 CAN 错误。", "ChassisFrame.drvErr1..4"), ({ chassis }) => quad(chassis, "drvErr"), (v) => tuple(v), (v) => !Array.isArray(v) || tupleMissing(v) ? "unknown" : Math.max(...v.map((x) => Math.abs(Number(x)))) > 50 ? "error" : Math.max(...v.map((x) => Math.abs(Number(x)))) > 20 ? "warn" : "normal"),
  ...
  simple("steer_err", "Steer error", "steer_err1..4", "deg", locationTip("四轮舵向包角误差，语义为 wrapped(steerCmd - steerFb)；ID1右前、ID2右后、ID3左前、ID4左后。", "绝对值≤30°。", ">30° 警告，>45° 异常。", "检查方向到位、编码器零点和机械卡滞，并按同 ID 对齐 command、feedback、PID output。", "ChassisFrame.steerErr1..4"), ({ chassis }) => quad(chassis, "steerErr"), (v) => tuple(v), (v) => !Array.isArray(v) || tupleMissing(v) ? "unknown" : Math.max(...v.map((x) => Math.abs(Number(x)))) > 45 ? "error" : Math.max(...v.map((x) => Math.abs(Number(x)))) > 30 ? "warn" : "normal"),
